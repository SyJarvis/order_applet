...
def index():
    ...
    list = StatDailySite.query.filter(StatDailySite.date >= date_from).filter(StatDailySite.date <=date_to).order_by(StatDailySite.id.asc()).all()
    data = resp_data['data']
    if list:
        for item in list:
            data['finance']['month'] += item.total_pay_money
            data['member']['month_new'] += item.total_new_member_count
            data['member']['total'] = item.total_new_member_count
            ...
            if getFormatDate(date=item.date,format="%Y-%m-%d") == date_to:
                ...
                data['shared']['today'] = item.total_order_count
