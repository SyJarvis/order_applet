...
from common.libs.Helper import ops_render, getFormatDate, iPagination, getDictFilterField, selectFilterObj
...
def food():
    ...
    pages = iPagination(page_params)
    offset = (page - 1) * app.config['PAGE_SIZE']
    list = query.order_by(StatDailyFood.id.desc()).offset(offset).limit(app.config['PAGE_SIZE'])
    date_list = []
    if list:
